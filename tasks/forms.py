...
class form_add_task(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['group_executor'].required = False
        self.fields['group_executor'].queryset = Group.objects.filter(
            name__in=['Администрация Орск', 'Администрация Оренбург'])
    ...
    def clean(self):
        cleaned_data = super(form_add_task, self).clean()
        condition1 = cleaned_data.get('group_executor')
        condition2 = cleaned_data['executor']

        if not self.errors:
            if condition1 is None and condition2 is None:
                raise forms.ValidationError(u'Заполните одно из полей "Исполнитель" или "Группа исполнителей"')
